...
// FIX v2.0: Unidad cambiada de GAL → LTS en toda la interfaz y lógica
...
export const FuelPanel: React.FC<FuelPanelProps> = ({ fleet = [], vendors = [] }) => {
          ...
          allLogs
            ...
            .reduce((acc, curr) =>
              curr.operation_type === 'IN'
                ? acc + Number(curr.liters)   // ← era gallons
                : acc - Number(curr.liters),  // ← era gallons
              0
            );
  ...
  const handleAction = async (e: React.FormEvent) => {
    ...
    const dbEntry = {
      ...
      liters:         Number(newRecord.liters),      // ← era gallons
    ...
    if (error) {
      ...
      setNewRecord({
        ...newRecord,
        aircraftId: '', liters: 0, ticketNumber: '', technician: '', hobbsAtCharge: 0,
      });
  ...
  // Capacidad de referencia: 10.000 LTS por tanque para la barra de progreso
    ...
    <div className="space-y-8 animate-in fade-in duration-700 text-left font-sans text-white">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="grid grid-cols-1 gap-2">
          <button
            onClick={() => { setActiveMode('HANGAR'); setIsAdding(true); }}
          ...
          <button
            onClick={() => { setActiveMode('AIRCRAFT'); setIsAdding(true); }}
        ...
        <table className="w-full font-mono text-left text-[10px]">
          <thead className="text-slate-600 uppercase tracking-widest bg-white/[0.02] border-b border-white/5">
            <tr>
              ...
              <th className="p-5 text-right">Firma</th>
            </tr>
          </thead>
        ...
        <div className="fixed inset-0 bg-black/98 backdrop-blur-xl z-[100] flex items-center justify-center p-4">
          <Card className="bg-[#050505] border border-[#E1AD01]/30 w-full max-w-xl shadow-2xl rounded-[2.5rem] overflow-hidden">
            <CardHeader className="bg-[#E1AD01] p-6 flex justify-between items-center text-black">
              ...
              <button onClick={() => setIsAdding(false)} className="text-black hover:rotate-90 transition-all">
                <X className="h-6 w-6" />
              </button>
            </CardHeader>
            <CardContent className="p-10 text-white">
              <form onSubmit={handleAction} className="space-y-6 font-mono">
                  ...
                  <div className="space-y-2">
                    ...
                    {activeMode === 'AIRCRAFT' ? (
                      <select
                        ...
                        onChange={e => setNewRecord({ ...newRecord, aircraftId: e.target.value })}
                      >
                      ...
                      <select
                        ...
                        className="w-full bg-[#0d0d0d] border border-white/10 p-4 rounded-xl
                                   text-white text-[10px] outline-none focus:border-[#E1AD01] uppercase
                                   [&>option]:bg-[#0d0d0d] [&>option]:text-white"
                        onChange={e => setNewRecord({ ...newRecord, vendorId: e.target.value })}
                      >
                ...
                <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      onChange={e => setNewRecord({ ...newRecord, liters: parseFloat(e.target.value) })}
                    />
                  </div>
                  <div className="space-y-2">
                    <label className="text-[9px] text-slate-500 font-black uppercase tracking-widest block">
                    ...
                    <input
                      ...
                      onChange={e => setNewRecord({ ...newRecord, ticketNumber: e.target.value })}
                    />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <select
                      className="w-full bg-[#0d0d0d] border border-white/10 p-4 rounded-xl
                                 text-white text-[10px] outline-none [&>option]:bg-[#0d0d0d] [&>option]:text-white"
                      onChange={e => setNewRecord({ ...newRecord, location: e.target.value as any })}
                    >
                ...
                {activeMode === 'AIRCRAFT' && (
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      onChange={e => setNewRecord({ ...newRecord, hobbsAtCharge: parseFloat(e.target.value) })}
                    />
                  </div>
                )}
                ...
                <button
                  ...
                  className="w-full bg-[#E1AD01] text-black font-black py-6 rounded-3xl mt-4 uppercase
                             text-[11px] tracking-[0.5em] shadow-2xl flex items-center justify-center gap-3
