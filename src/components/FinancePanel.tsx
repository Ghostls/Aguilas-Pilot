...
export const FinancePanel: React.FC<FinancePanelProps> = ({
  ...
  // ─── FUNCIÓN: Exportar Cajas Chicas a Excel ──────────────────────────
  ...
  const handleExportCajasExcel = () => {
    ...
    rows.push('REPORTE DE CAJAS CHICAS - ÁGUILAS PILOT');

    rows.push(`Fecha de exportación: ${new Date().toLocaleDateString('es-VE', {day:'2-digit', month:'2-digit', year:'numeric', hour:'2-digit', minute:'2-digit'})}`);
    ...
    cajas.forEach(caja => {
      ...
      const movsCaja = movCajas.filter(m => m.caja_id === caja.id);

      

      rows.push(`CAJA: ${caja.nombre.toUpperCase()}`);

      rows.push('Fecha,Tipo,Moneda,Monto,Concepto,Referencia,Registrado por');
      ...
      if (movsCaja.length === 0) {
        ...
        movsCaja.forEach(m => {
          ...
          const tipo = m.tipo === 'ENTRADA' ? 'INGRESO' : 'EGRESO';

          const monto = m.moneda === 'BS' 
      ...
      rows.push('Moneda,Total Ingresos,Total Egresos,Saldo');

      

      (['USDT','ZELLE','CASH','BS'] as PaymentMethod[]).forEach(moneda => {

        const ingresos = movsCaja
        ...
        if (ingresos > 0 || egresos > 0) {
          ...
          rows.push([
            ...
            `${prefix}${ingresos.toFixed(2)}`,

            `${prefix}${egresos.toFixed(2)}`,

            `${prefix}${saldo.toFixed(2)}`
    ...
    link.setAttribute('download', `Cajas_Chicas_Aguilas_${new Date().toISOString().split('T')[0]}.csv`);
  ...
  // ─── FUNCIÓN: Exportar Caja Individual a Excel ──────────────────────────
  ...
  const handleExportCajaIndividual = (cajaId: string) => {
    ...
    const movsCaja = movCajas.filter(m => m.caja_id === cajaId);

    const rows = [

      `CAJA: ${caja.nombre.toUpperCase()}`,

      `Fecha de exportación: ${new Date().toLocaleDateString('es-VE', {day:'2-digit', month:'2-digit', year:'numeric', hour:'2-digit', minute:'2-digit'})}`,

      '',

      'Fecha,Tipo,Moneda,Monto,Concepto,Referencia,Registrado por'
    ...
    link.setAttribute('download', `Caja_${caja.nombre.replace(/\s+/g, '_')}_${new Date().toISOString().split('T')[0]}.csv`);
    ...
    <div className="space-y-8 animate-in fade-in duration-700 font-mono text-white">
      ...
      {activeTab === 'CAJAS' && (
        ...
        <div className="space-y-6">
          ...
          {/* BOTÓN EXPORTAR TODAS LAS CAJAS A EXCEL */}

          <div className="flex justify-end">

            <button

              onClick={handleExportCajasExcel}
                ...
                <div className={`lg:col-span-8 ${glass} rounded-3xl overflow-hidden flex flex-col`}>
                  ...
                  <div className="p-6 border-b border-white/5">
                    ...
                    <div className="flex justify-between items-center mb-3">
                      ...
                      {/* BOTÓN EXPORTAR CAJA INDIVIDUAL */}

                      <button

                        onClick={() => handleExportCajaIndividual(cajaActiva!)}
